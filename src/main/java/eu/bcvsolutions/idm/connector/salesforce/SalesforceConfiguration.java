...
public class SalesforceConfiguration extends AbstractConfiguration {
	...
	public void validate() {
		...
		if (StringUtil.isBlank(authUrl)) {
			throw new ConfigurationException("Auth URL must not be blank!");
		}
		if (StringUtil.isBlank(grandType)) {
			throw new ConfigurationException("Grand type must not be blank!");
		}
		if (clientId == null) {
